...
Terraform init
```
Checks the Terraform configuration files for syntax errors and validate them against the Terraform language specification.
This ensures that the configuration files are correctly written and can be processed by Terraform.
```HCL
Terraform validate
```
Create an execution plan based on the Terraform configuration. Calculate the changes that will be made to the infrastructure to achieve the desired state specified in the configuration files (main.tf, variables.tf, etc.).
Provides a preview of what Terraform will do when you run terraform apply.
...
Apply the changes described in the execution plan generated by terraform plan to the infrastructure.
this creates, updates, or deletes resources as necessary to achieve the desired state specified in the configuration files.
It interacts with the cloud provider (in this case, Azure) to make the necessary changes to the  infrastructure.
 ``` 
 Terraform apply
 ```
CAUTION: Do not run this unless you are testing the Terraform configurations.
this will remove all infrastructure resources created by Terraform, effectively deleting them from the cloud provider (Azure).
It's important to use this command with caution, as it irreversibly deletes resources and can lead to data loss if not used carefully.
check with your peer team memeber or lead before running this command as a best practice.. probably create a PR request before running? 
...
That's all Fokes!
To customise location or other componets start playing with the Variables file 
